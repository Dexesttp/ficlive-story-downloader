...
module.exports.retrieve_ficlive_data = async function (
  ...
  if (options.show_output)
    console.log(
      `${folder_name} - [[[Node data]]] - ${node_url_from_id(node_id)}`
    );
  /** @type {utils.NodeData} */
  const data = await get_json_data_or_download(
    `${story_directory_path}/node.json`,
  ...
  for (let i = 0; i < data.bm.length; ++i) {
    const bm = data.bm[i];
    const next_bm = data.bm.find(
      (d, ii) => ii > i && !d.title.startsWith("#special")
    );
    if (options.show_output)
      console.log(
        `${folder_name} - ${bm.title} - ${content_url_from_bm(
          node_id,
          bm,
          next_bm
        )}`
    ...
    await get_json_data_or_download(
      `${story_directory_path}/chapter_${bm.id}.json`,
